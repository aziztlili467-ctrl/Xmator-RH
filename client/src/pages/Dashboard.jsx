...
function Star({ size, gold }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" className="block shrink-0" style={gold ? { filter: 'drop-shadow(0 0 4px rgba(227,185,77,0.55))' } : undefined}>
      <path fill={gold ? '#e9c25a' : 'rgba(148,163,199,0.22)'} d="M12 2l2.9 6.3 6.9.7-5.2 4.6 1.5 6.8L12 17.8l-6.1 3.6 1.5-6.8L2.2 9l6.9-.7z" />
    </svg>
  );
}
function Stars({ value, size = 14, label }) {
  ...
  return (
    <span className="inline-flex items-center gap-1.5" title={`${label || 'Note'} : ${v}/5`}>
      ...
      <span className="num text-[10px] font-bold" style={{ color: '#e9c25a' }}>{v}/5</span>
    </span>
  );
}
...
function KpiWidget({ title, valueEl, sub, icon, chip, glow = 'rgba(227,185,77,0.5)', color = '#e3b94d', spark, delay = 0, onClick }) {
  return (
    <div
      role="button"
      tabIndex={0}
  ...
  return (
    <div className="flex flex-wrap items-center gap-2.5">
      ...
      <button type="button" className="day-pill" onClick={onOpenAlertes} title="Alertes critiques du jour — ouvrir le panneau">
        <span className="live-dot" style={{ background: critiques ? 'var(--dp-rose)' : 'var(--dp-emerald)', boxShadow: `0 0 10px ${critiques ? 'rgba(251,113,133,.9)' : 'rgba(52,211,153,.9)'}` }} />
        <div className="text-start leading-tight">
          <p className="clock-value text-[13px]" style={{ color: critiques ? '#fda4af' : '#6ee7b7' }}>{critiques}</p>
...
export default function Dashboard({ demo: demoProp = false }) {
            ...
            {cfg.rows.map(({ e, val }, i) => (
              <div key={`${e.nom}-${i}`} className="rise rounded-xl px-3 py-2.5" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(148,163,199,0.09)', '--d': `${i * 40}ms` }}>
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="num shrink-0 text-[13px] font-extrabold" style={{ color: drawer.key === 'presence' ? 'var(--dp-emerald)' : '#ffe9a8' }}>
            ...
            {rows.map((e, i) => (
              <div key={e.id} className="rise rounded-xl px-3 py-2.5" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(148,163,199,0.09)', '--d': `${i * 40}ms` }}>
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="num shrink-0 text-[13px] font-extrabold" style={{ color: '#ffe9a8' }}>{fmtJours(e[field])} j</span>
                </div>
        ...
        body: (
          <div className="space-y-4">
                  ...
                  return (
                    <button key={e.id} type="button" onClick={() => setDrawer({ type: 'employe', id: e.id })} className="w-full rounded-xl px-3 py-2.5 text-start" style={{ background: 'rgba(251,113,133,0.06)', border: '1px solid rgba(251,113,133,0.25)' }}>
                      <span className="flex items-center justify-between gap-2">
                        ...
                        <span className="num text-[12px] font-extrabold" style={{ color: solde < 0 ? '#fda4af' : 'var(--dp-amber)' }}>{solde < 0 ? `${fmtJours(solde)} j` : `${fmtJours(solde)} j restants`}</span>
                      </span>
            ...
            <section>
              ...
              <div>
                <p className="tl-item" style={{ '--tl-color': 'var(--dp-cobalt)', '--tl-glow': 'rgba(76,125,255,.6)' }}>
                ...
                <p className="tl-item" style={{ '--tl-color': 'var(--dp-rose)', '--tl-glow': 'rgba(251,113,133,.6)' }}>
                ...
                <p className="tl-item" style={{ '--tl-color': 'var(--dp-amber)', '--tl-glow': 'rgba(251,191,36,.6)' }}>
  ...
  // ============================================================================
  return (
    <div data-theme="dark-premium" className="relative -mx-3 -mt-4 -mb-6 px-3 pb-8 pt-4 sm:-mx-6 sm:px-6 min-h-full" style={{ background: 'var(--bg-app-gradient)', backgroundColor: 'var(--dp-ink)' }}>
      {/* ============================ BANDEAU COCKPIT ============================ */}
          ...
          <div className="panel rule-gold p-4">
            ...
            <div className={`mt-4 space-y-4 ${filtresOuverts ? 'block' : 'hidden'} xl:block`}>
              ...
              {matriculeIntrouvable && (
                <p className="rounded-xl px-3.5 py-2.5 text-[11.5px] font-semibold" style={{ background: 'rgba(251,113,133,0.1)', border: '1px solid rgba(251,113,133,0.35)', color: '#fda4af' }}>
              ...
              <>
                ...
                <div className="grid grid-cols-1 gap-4 2xl:grid-cols-[1fr_1.15fr_1.35fr]">
                  ...
                  <div className="panel rise p-5" style={{ '--d': '120ms' }} onClick={() => setDrawer({ type: 'kpi', key: 'presence' })}>
                    ...
                    <div className="grid grid-cols-2 items-center gap-x-2 gap-y-4 pt-1">
                      <RadialGauge pct={data.barometres.presence} label="Présence (heures)" sub={`${fmtHeures(k.heures_travaillees)} / ${fmtHeures(k.heures_legales)}`} color="#e3b94d" size={150} />
                      <RadialGauge pct={data.barometres.ponctualite} label="Ponctualité" sub={`${k.journees_presence - k.retards} / ${k.journees_presence} j sans retard`} color="#34d399" size={150} />
                      <div className="col-span-2 grid grid-cols-3 gap-2.5">
                        <RadialGauge mini pct={data.barometres.jours_presence} label="Jours présents" color="#5eead4" size={96} />
                        <RadialGauge mini pct={data.barometres.conge} label="Congés" color="#4c7dff" size={96} />
                        <RadialGauge mini pct={data.barometres.maladie} label="Maladie" color="#fb7185" size={96} />
                      </div>
                    </div>
                    <div className="mt-3 flex items-center justify-center gap-3 text-[10.5px]" style={{ color: 'var(--dp-text-3)' }}>
                      <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full" style={{ background: '#34d399' }} /> ≥ 100 %</span>
                      <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full" style={{ background: '#fbbf24' }} /> 90–99 %</span>
                      <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full" style={{ background: '#fb7185' }} /> &lt; 90 %</span>
                    </div>
                  </div>
                ...
                <div className={`grid grid-cols-1 items-stretch gap-4 ${zoomCombo ? 'xl:grid-cols-1' : 'xl:grid-cols-2'}`}>
                  ...
                  >
                    ...
                    <div className="mt-3 flex flex-wrap items-center justify-center gap-x-4 gap-y-1 text-[10.5px]" style={{ color: 'var(--dp-text-2)' }}>
                      <span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-sm" style={{ background: 'rgba(148,163,199,0.45)' }} /> Heures légales</span>
                      ...
                      <span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full" style={{ background: '#34d399' }} /> ≥ 100 %</span>
                      <span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full" style={{ background: '#fbbf24' }} /> 90–99 %</span>
                      <span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full" style={{ background: '#fb7185' }} /> &lt; 90 %</span>
                    ...
                    <div className="panel rise p-5" style={{ '--d': '260ms' }}>
                      ...
                      <ResponsiveContainer width="100%" height={262}>
                        <AreaChart data={stackData} margin={{ top: 6, right: 6, left: -16, bottom: 0 }}>
                          ...
                          <CartesianGrid strokeDasharray="3 4" stroke="rgba(148,163,199,0.12)" vertical={false} />
                          <XAxis dataKey="label" tick={{ fontSize: 10, fill: 'var(--dp-text-3)' }} tickLine={false} axisLine={{ stroke: 'rgba(148,163,199,0.18)' }} />
                            ...
                            content={({ active, payload, label }) => {
                              ...
                              return (
                                <div className="tip" style={{ position: 'relative' }}>
                                  ...
                                  {payload.map((p) => (
                                    <div key={p.name} className="tip-row"><span>{p.name}</span><b style={{ color: p.color }}>{p.value} %</b></div>
                                  ))}
                                </div>
                              );
                            }}
                            cursor={{ stroke: 'rgba(227,185,77,0.5)', strokeDasharray: '3 3' }}
                          />
                          <Legend wrapperStyle={{ fontSize: 11 }} />
                          {Object.entries(STACK_COLORS).map(([name, color]) => (
                            <Area key={name} type="monotone" dataKey={name} stackId="1" stroke={color} fill={`url(#stack-${name})`} strokeWidth={1.2} />
                          ))}
                        </AreaChart>
                      </ResponsiveContainer>
                ...
                <div className="grid grid-cols-1 gap-4 2xl:grid-cols-[1.1fr_1fr_0.9fr]">
                  <div className="panel rise p-5" style={{ '--d': '300ms' }}>
                    ...
                    <div className="max-h-[268px] overflow-y-auto pe-1">
                      <StairsStatsDiagram data={retardData} colors={{ retards: '#fbbf24', sorties: '#a78bfa' }} />
                    </div>
                  </div>
                      ...
                      <div className="space-y-2.5">
                        {topRetards.map((e, i) => (
                          <button
                            ...
                            onClick={() => setDrawer({ type: 'employe', id: e.id })}
                          >
                            <span className="num flex shrink-0 items-center justify-center rounded-lg text-[11px] font-black" style={{ width: 26, height: 26, background: i === 0 ? 'rgba(251,113,133,0.16)' : 'rgba(148,163,199,0.09)', color: i === 0 ? '#fda4af' : 'var(--dp-text-3)', border: i === 0 ? '1px solid rgba(251,113,133,0.4)' : '1px solid rgba(148,163,199,0.12)' }}>{i + 1}</span>
                            <span className="min-w-0 flex-1">
                              <span className="flex items-center justify-between gap-2">
                                ...
                                <span className="num shrink-0 text-[11px] font-extrabold" style={{ color: '#fda4af' }}>{e.retards} j · {fmtDur(e.retard_secondes)}</span>
                              </span>
                              <span className="minibar mt-1.5 block"><i style={{ width: `${Math.min(100, (e.retard_secondes / maxRetardsSec) * 100)}%`, background: 'linear-gradient(90deg, #fb7185, rgba(251,113,133,0.2))', boxShadow: '0 0 8px rgba(251,113,133,0.5)' }} /></span>
                            </span>
                          </button>
                        ))}
                      </div>
                  ...
                  <div className="panel rule-gold rise flex flex-col p-5" style={{ '--d': '380ms' }}>
                    ...
                    <div className="flex flex-1 flex-col justify-center gap-3">
                      {[
                        { l: 'Retards (entrées)', pct: data.barometres.ponctualite, v: `${k.journees_presence - k.retards}/${k.journees_presence}`, stars: k.note_retards, color: '#fb7185' },
                        { l: 'Sorties conformes', pct: data.barometres.sorties_conformes, v: `${k.journees_presence - k.departs_anticipe}/${k.journees_presence}`, stars: k.note_sorties, color: '#5eead4' },
                      ].map((row) => (
                ...
                {(data.alertes || []).length > 0 && (
                  <div className="panel rise p-5" style={{ borderColor: 'rgba(251,191,36,0.35)', '--d': '420ms' }}>
                    <PanelHead
                      title={<span className="flex items-center gap-2" style={{ color: '#fcd34d' }}><IconAlert /> Alertes de solde ({data.alertes.length})</span>}
                      ...
                      <tbody>
                        ...
                        {employesFiltres.map((e) => (
                          <tr key={e.id} onClick={() => setDrawer({ type: 'employe', id: e.id })}>
                            ...
                            <td><span className="num font-semibold" style={{ color: '#6ee7b7' }} title={`${e.jours_presents} j sur ${e.jours_ouvrables} j légaux`}>{e.jours_presents} j</span></td>
                            <td className="num">{e.jours_conge ? fmtJours(e.jours_conge) : '—'}</td>
                            <td className="num">{e.jours_maladie ? fmtJours(e.jours_maladie) : '—'}</td>
                            <td>{e.jours_absence ? <span className="num font-bold" style={{ color: 'var(--dp-amber)' }}>{e.jours_absence} j</span> : <span className="num" style={{ color: '#3f9d77' }}>0</span>}</td>
                            <td>
                              {e.retards ? (
                                <span className="num font-bold" style={{ color: '#fda4af' }} title={`cumul ${fmtDur(e.retard_secondes)}`}>{e.retards} j</span>
                              ) : <span className="num" style={{ color: '#3f9d77' }}>0</span>}
                            </td>
                            <td className="num">{e.departs_anticipe || '—'}</td>
                            <td><span className="num font-extrabold" style={{ color: e.solde_conge < 0 ? '#fda4af' : e.solde_conge < 5 ? 'var(--dp-amber)' : '#6ee7b7' }}>{fmtJours(e.solde_conge)} j</span></td>
                            <td><span className="num font-extrabold" style={{ color: e.solde_maladie < 0 ? '#fda4af' : 'var(--dp-cobalt)' }}>{fmtJours(e.solde_maladie)} j</span></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
