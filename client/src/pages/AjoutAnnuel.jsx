...
export default function AjoutAnnuel() {
  ...
  const submit = async (e) => {
    ...
    try {
      if (mode === 'individuel') {
        ...
        setSuccess(`Ajout annuel de ${fmtJours(j)} j crédité pour ${selected.nom} ${selected.prenom}. Nouveau solde : ${fmtJours(mvt.solde_apres)} j.`);
        setSelected(null);
      } else {
        if (!preview || preview.length === 0) return setError('Aucun employé ne correspond au critère choisi.');
        const r = await api.ajoutAnnuelMasse({
          jours: j,
          date_operation: date,
          motif,
        ...
        setSuccess(`Ajout annuel de ${fmtJours(j)} j crédité pour ${r.count} employé(s).`);
      }
      setJours('');
      setMotif('');
      setDate(today());
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="card flex gap-1 p-1">
        {['individuel', 'masse'].map((m) => (
          ...
          >
            {m === 'individuel' ? 'Pour un employé' : 'En masse (tous / par catégorie)'}
          </button>
        ))}
      </div>
      ...
      <form onSubmit={submit} className="card space-y-5 p-6">
        ...
        <Field label="Remarque / motif" hint="Optionnel">
          <textarea className="input" rows={2} value={motif} onChange={(e) => setMotif(e.target.value)} placeholder="Ex. Droit annuel 2027" />
        </Field>

        <div className="flex justify-end">
          <button className="btn-primary" disabled={saving}>
            {saving ? 'Crédit en cours…' : mode === 'individuel' ? 'Ajouter le solde' : 'Créditer tous les employés sélectionnés'}
          </button>
        </div>
      </form>
    </div>
