...
export default function DonutPro({ data = [], size = 220, thickness = 26, unit = 'j', onSelect }) {
  ...
  // Valeur centrale : taille adaptée au nombre de chiffres pour ne jamais
  // déborder de la zone lisible du centre du donut.
  const centerValue = act ? `${Math.round(act.frac * 1000) / 10} %` : fmtNum(total);
  const digits = Math.max(1, centerValue.replace(/\D/g, '').length);
  const centerFs = digits <= 4 ? 22 : digits === 5 ? 18 : digits === 6 ? 15 : 13;
  ...
  return (
    ...
    >
      ...
      {/* Centre */}
      <div className="absolute inset-0 flex flex-col items-center justify-center text-center pointer-events-none" style={{ padding: '16%' }}>
        <span className="donut-center-val font-extrabold leading-none" style={{ color: act ? act.color : 'var(--dp-text)', fontSize: centerFs }}>
          {centerValue}
        </span>
        <span className="mt-1 max-w-full truncate text-[8px] font-bold uppercase tracking-[0.12em]" style={{ color: 'var(--dp-text-3)' }}>
