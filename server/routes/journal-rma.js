...
router.get('/', (req, res) => {
  ...
  for (const r of rows) {
    ...
    if (r.demi_journee && r.code === CODE_RMA_CONGE) {
...
router.post('/import', (req, res) => {
  ...
  const result = tx();
  // Déduction automatique du solde pour les CA nouvellement importés (hors ceux déjà couverts par une demande acceptée)
  let mouvementsCrees = 0;
  for (const ca of nouveauxCA) {
    if (demandeAccepteeCouvre(ca.employe_id, ca.date)) continue;
    const existe = db.prepare("SELECT id FROM mouvements WHERE employe_id = ? AND date_operation = ? AND type_operation = 'prelevement' AND solde_type = 'conge' AND motif LIKE 'RMA%' LIMIT 1").get(ca.employe_id, ca.date);
    if (existe) continue;
    try {
      insertMouvement({
        employe_id: ca.employe_id,
        type_operation: 'prelevement',
        date_operation: ca.date,
        jours: 1,
        motif: `RMA \u2014 Cong\u00E9 ${CODE_RMA_CONGE} du ${ca.date} \u2014 Mat ${ca.matricule} (auto)`,
        solde_type: 'conge',
      });
      mouvementsCrees++;
    } catch {}
...
router.delete('/', (req, res) => {
  ...
  // Pré-sélection des CA à restaurer au solde (si un mouvement RMA existe pour cette date)
  let selSql = "SELECT employe_id, matricule, date FROM codes_importes WHERE code = ? AND date >= ? AND date <= ?";
  const selParams = [CODE_RMA_CONGE, debut, fin];
  if (matricule) {
  ...
  // Restauration du solde pour chaque CA qui avait généré un mouvement RMA per-date
  ...
  for (const ca of caASupprimer) {
    ...
    try {
      insertMouvement({
        ...
        jours: 1,
        motif: `RMA \u2014 restauration cong\u00E9 ${CODE_RMA_CONGE} du ${ca.date} \u2014 Mat ${ca.matricule} (suppression)`,
        solde_type: 'conge',
      });
      mouvementsRestaures++;
    } catch {}
